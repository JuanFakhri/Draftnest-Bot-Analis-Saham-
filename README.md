...
- **BSJP (Beli Sore, Jual Pagi)** — peluang historis overnight gap + **2 strategi entry ter-backtest** (RSI Pullback, Momentum Breakout) dengan win rate & rata gain semalam yang bisa **dihitung ulang live per filter** (skor/volume/sektor); gabungan **S1 ATAU/DAN S2**. **Sinyal di-refresh otomatis tiap hari bursa** (cron setelah tutup) + tombol **Cek Sinyal Live** per emiten (ambil harga terkini via proxy, hitung sinyal sekarang). **Bukan jaminan** — risiko gap-down; syarat "Foreign Flow" Strategi 1 dilewati (tak tersedia dari sumber data)
